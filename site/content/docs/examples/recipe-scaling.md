...
summary: "Scale a recipe from 1 to 4 loaves with metric-to-US conversion and cost analysis."
weight: 40
---

Converting a European bread recipe from metric to US customary units, while scaling from 1 loaf to 4 loaves.
...
Converting a European bread recipe from metric to US customary units,
while scaling from 1 loaf to 4 loaves for a dinner party.

## Original Recipe (1 loaf, metric)
...
## Scaled Quantities (Still Metric)
...
Using standard conversions (grams per unit):
...
Original proofing temp in Celsius, converted to Fahrenheit:
...
Larger batches may need slightly longer rise times:
...
- Batch scaling with a scale factor
- Unit conversion (Celsius to Fahrenheit via `in`)
- Manual unit conversion (grams to cups/tsp)
