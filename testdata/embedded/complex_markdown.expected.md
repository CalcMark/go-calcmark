...
```calcmark
servers = 12 → 12
cost_per_server = 450 USD → $450.00
monthly_server_cost = servers * cost_per_server → $5,400.00
```
...
```calcmark
storage_tb = 50 → 50
cost_per_tb = 23 USD → $23.00
monthly_storage = storage_tb * cost_per_tb → $1,150.00
```
...
```calcmark
egress_tb = 10 → 10
cost_per_egress_tb = 87 USD → $87.00
monthly_network = egress_tb * cost_per_egress_tb → $870.00
```
...
```calcmark
bandwidth_gbps = 40 → 40
cost_per_gbps = 15 USD → $15.00
monthly_bandwidth = bandwidth_gbps * cost_per_gbps → $600.00
```
