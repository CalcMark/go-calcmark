...
## Addendum 2026-09-05 — re-plan after v2.2.12

The April requirements stand. The codebase they were written against does not, so the following amendments apply to the refreshed plan (`docs/plans/2026-09-05-001-feat-named-tables-and-arrays-plan.md`):

- **Embedded mode (v2.2.0).** Sources may be standard markdown with ```` ```cm ```` fences. Tables and their `<!-- table: … -->` directive live in the markdown *outside* fences, which `NewDocumentEmbedded` projects as `TextBlock`s. Table extraction therefore stays TextBlock-based and works for both flat and Embedded sources with no extra dispatch.
- **Directive syntax stays an HTML comment.** Reconsidered against first-class frontmatter (#136): the directive is per-table, not per-document, so frontmatter is the wrong home. An HTML comment is invisible in every markdown viewer (GitHub, Obsidian, Hugo) and in cmw's rendered preview — which is the point: the *table* is the visible artifact. cmw will render a small "named table" badge on the widget (Workstream C8) so the name is discoverable while editing.
- **Positions are mandatory.** Since #164 every primary expression carries a range (`ast.SetRangeIfMissing` at the `parsePrimary` boundary) and runtime errors are positioned at the failing node. `MemberAccess` gets its range for free from that boundary; table-extraction diagnostics must carry the directive line and the offending cell's column so cmw can underline them.
- **Exemplar for a new type is Period (v2.0), not Fraction.** Period was the last type to go through every layer; its PR (#145) is the current cross-layer template. The Fraction checklist remains the layer list.
- **LSP completion is in scope (R16, new).** After `name.` where `name` is a registered table, the LSP completes the table's column names. Completions for table names come from the existing variable path because tables live in the environment.
- **cmw consumer (R17, new).** The cmw table widget already interpolates `{{var}}` in cells and renders GFM alignment; R15 (array-in-cell) must produce per-row values through the same `InterpolatedSource` path so cmw needs no new projection.
