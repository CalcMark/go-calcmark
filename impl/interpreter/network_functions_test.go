...
func TestConvertQuantityToMegabytes(t *testing.T) {
	tests := []struct {
		value       float64
		unit        string
		expectedMB  float64
		...
		{1024, "byte", 0.0009765625, false},  // 1 KB in MB
		{1, "kilobyte", 0.0009765625, false}, // 1 KB
		{1, "megabyte", 1.0, false},          // 1 MB
		{1, "gigabyte", 1024.0, false},       // 1 GB
		{0.5, "gigabyte", 512.0, false},      // 0.5 GB
		{1, "MB", 1.0, false},                // Abbreviation
		{1, "meters", 0, true},               // Invalid unit
	}
	...
	for _, tt := range tests {
		t.Run(tt.unit, func(t *testing.T) {
			...
			result, err := convertQuantityToMegabytes(q)

			if tt.expectError {
				if err == nil {
					t.Errorf("Expected error for unit '%s', got none", tt.unit)
				}
				return
			}
			...
			diff := result - tt.expectedMB
			if diff < 0 {
				diff = -diff
			}
			if diff > 0.0001 {
				t.Errorf("Expected %v MB, got %v MB", tt.expectedMB, result)
			}
		})
	}
}
