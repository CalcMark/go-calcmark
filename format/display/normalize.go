...
// unitSymbolAliases maps common unit variations to canonical display symbols.
var unitSymbolAliases = map[string]string{
...
func normalizeUnitSymbol(unit string) string {
	if symbol, ok := unitSymbolAliases[strings.ToLower(unit)]; ok {
		return symbol
	}
	return unit
}
