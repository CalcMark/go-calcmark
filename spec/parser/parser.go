...
// parseFunctionCall parses function calls (avg, sqrt, etc.)
func (p *Parser) parseFunctionCall() (ast.Node, error) {
	funcToken := p.advance()

	// Map function token types to canonical names
	funcName := ""
	switch funcToken.Type {
	case lexer.FUNC_AVG, lexer.FUNC_AVERAGE_OF:
		funcName = "avg"
	case lexer.FUNC_SQRT, lexer.FUNC_SQUARE_ROOT_OF:
		funcName = "sqrt"
	default:
		return nil, &ParseError{
			Message: fmt.Sprintf("Unknown function token %s", funcToken.Type),
			Line:    funcToken.Line,
			Column:  funcToken.Column,
		}
	}

	// For "average of" and "square root of", arguments follow directly
	// For "avg(" and "sqrt(", expect parentheses
	expectParens := funcToken.Type == lexer.FUNC_AVG || funcToken.Type == lexer.FUNC_SQRT
	...
	if expectParens {
		...
		if p.currentToken().Type != lexer.LPAREN {
			return nil, &ParseError{
				Message: fmt.Sprintf("Expected '(' after %s", funcName),
		...
		for {
			...
			if p.currentToken().Type == lexer.COMMA {
				...
				return nil, &ParseError{
					Message: fmt.Sprintf("Expected ',' or ')' in function arguments, got %s", p.currentToken().Type),
					Line:    p.currentToken().Line,
					Column:  p.currentToken().Column,
				}
			}
		...
		// Multi-token functions: "average of 1, 2, 3" or "square root of 25"
		// Parse arguments separated by commas until end of line
		for {
...
func (p *Parser) parsePrimary() (ast.Node, error) {
	...
	if token.Type == lexer.NUMBER {
		...
		return &ast.NumberLiteral{
			Value:      token.Value,
			SourceText: token.OriginalText,
			Range:      tokenToRange(token),
		}, nil
	}

	if token.Type == lexer.CURRENCY || token.Type == lexer.QUANTITY {
		p.advance()
		// Token value is "value:unit" (e.g., "100:$" or "100:USD")
		parts := strings.SplitN(token.Value, ":", 2)
		value := parts[0]
		unit := parts[1]
		return &ast.CurrencyLiteral{
			Value:      value,
			Symbol:     unit, // For now, storing unit as Symbol (will be renamed in future)
			SourceText: token.OriginalText,
			Range:      tokenToRange(token),
		}, nil
	}
	...
	if token.Type == lexer.FUNC_AVG || token.Type == lexer.FUNC_SQRT ||
		token.Type == lexer.FUNC_AVERAGE_OF || token.Type == lexer.FUNC_SQUARE_ROOT_OF {
