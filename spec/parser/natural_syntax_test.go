package parser

import (
	"testing"

	"github.com/CalcMark/go-calcmark/spec/ast"
)

func TestNaturalSyntaxRateFunctions(t *testing.T) {
	tests := []struct {
		name        string
		input       string
		expectError bool
		checkFunc   func(*testing.T, []ast.Node)
	}{
		{
			name:        "over keyword - accumulate",
			input:       "100 MB/s over 1 day\n",
			expectError: false,
			checkFunc: func(t *testing.T, nodes []ast.Node) {
				if len(nodes) != 1 {
					t.Fatalf("Expected 1 node, got %d", len(nodes))
				}
				fc, ok := nodes[0].(*ast.FunctionCall)
				if !ok {
					t.Fatalf("Expected FunctionCall, got %T", nodes[0])
				}
				if fc.Name != "accumulate" {
					t.Errorf("Expected function 'accumulate', got '%s'", fc.Name)
				}
				if len(fc.Arguments) != 2 {
					t.Errorf("Expected 2 arguments, got %d", len(fc.Arguments))
				}
			},
		},
		{
			name:        "over with per keyword rate",
			input:       "5 GB per day over 1 year\n",
			expectError: false,
			checkFunc: func(t *testing.T, nodes []ast.Node) {
				fc, ok := nodes[0].(*ast.FunctionCall)
				if !ok {
					t.Fatalf("Expected FunctionCall, got %T", nodes[0])
				}
				if fc.Name != "accumulate" {
					t.Errorf("Expected 'accumulate', got '%s'", fc.Name)
				}
			},
		},
		{
			name:        "per conversion context",
			input:       "5 million/day per second\n",
			expectError: false,
			checkFunc: func(t *testing.T, nodes []ast.Node) {
				fc, ok := nodes[0].(*ast.FunctionCall)
				if !ok {
					t.Fatalf("Expected FunctionCall, got %T", nodes[0])
				}
				if fc.Name != "convert_rate" {
					t.Errorf("Expected 'convert_rate', got '%s'", fc.Name)
				}
				if len(fc.Arguments) != 2 {
					t.Errorf("Expected 2 arguments, got %d", len(fc.Arguments))
				}
			},
		},
		{
			name:        "cost rate over time",
			input:       "$0.10/hour over 30 days\n",
			expectError: false,
			checkFunc: func(t *testing.T, nodes []ast.Node) {
				fc, ok := nodes[0].(*ast.FunctionCall)
				if !ok {
					t.Fatalf("Expected FunctionCall, got %T", nodes[0])
				}
				if fc.Name != "accumulate" {
					t.Errorf("Expected 'accumulate', got '%s'", fc.Name)
				}
			},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			nodes, err := Parse(tt.input)

			if tt.expectError && err == nil {
				t.Error("Expected error but got none")
				return
			}

			if !tt.expectError && err != nil {
				t.Fatalf("Unexpected error: %v", err)
			}

			if tt.checkFunc != nil {
				tt.checkFunc(t, nodes)
			}

			t.Logf("✓ Parsed: %+v", nodes[0])
		})
	}
}
